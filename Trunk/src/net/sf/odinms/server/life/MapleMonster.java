...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
        ...
        } else if (!isBoss() || ((getId() > 9300183 && getId() < 9300216) && SpecialStuff.getInstance().isDojoMap(getMap().getId()))) {
    ...
    private void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            if (exp > 0) {
                ...
                long personalExpL = personalExp * Rates.getExpRate(attacker);
                if (personalExpL > Integer.MAX_VALUE) {
                    personalExp = Integer.MAX_VALUE - 1;
                }
                personalExp = (int) personalExpL;
                if (personalExp < 0) {
                    personalExp = Integer.MAX_VALUE - 1;
                }
            ...
            if (toSpawn.contains(9300216)) {
                ...
                if (killer.getParty() != null) {
                    for (MaplePartyCharacter mpc : killer.getParty().getMembers()) {
                        MapleCharacter oo = killer.getClient().getChannelServer().getPlayerStorage().getCharacterByName(mpc.getName());
                        if (oo != null) {
                            if (oo.getMapId() == killer.getMapId()) {
                                oo.showMessage(5, "You received " + oo.addDojoPointsByMap() + " training points. Your total training points score is now " + oo.getDojoPoints() + ".");
                            }
                        }
                    }
                } else {
                    killer.showMessage(5, "You received " + killer.addDojoPointsByMap() + " training points. Your total training points score is now " + killer.getDojoPoints() + ".");
                }

                MapleReactor reactor = killer.getMap().getReactorByName("door");
                if (reactor.getState() != (byte) 1) {
                    reactor.delayedHitReactor(killer.getClient(), 500);
                }
            }
