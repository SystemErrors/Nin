...
public class MapleMiniGame extends AbstractMapleMapObject {
    ...
    public void addVisitor(MapleCharacter challenger) {
        ...
        if (GameType.equals("omok")) {
            this.getOwner().getClient().getSession().write(ShopAndGamePackets.getMiniGameNewVisitor(challenger, 1));
            this.getOwner().getMap().broadcastMessage(ShopAndGamePackets.addOmokBox(owner, 2, 0));
        }
        if (GameType.equals("matchcard")) {
            this.getOwner().getClient().getSession().write(ShopAndGamePackets.getMatchCardNewVisitor(challenger, 1));
            this.getOwner().getMap().broadcastMessage(ShopAndGamePackets.addMatchCardBox(owner, 2, 0));
        }
    }
    ...
    public void removeVisitor(MapleCharacter challenger) {
        if (visitor == challenger) {
            visitor = null;
            this.getOwner().getClient().getSession().write(ShopAndGamePackets.getMiniGameRemoveVisitor());
            if (GameType.equals("omok")) {
                this.getOwner().getMap().broadcastMessage(ShopAndGamePackets.addOmokBox(owner, 1, 0));
            }
            if (GameType.equals("matchcard")) {
                this.getOwner().getMap().broadcastMessage(ShopAndGamePackets.addMatchCardBox(owner, 1, 0));
            }
        }
    }
    ...
    public void setOwnerPoints() {
        ...
        if (ownerpoints + visitorpoints == matchestowin) {
            if (ownerpoints == visitorpoints) {
                this.broadcast(ShopAndGamePackets.getMatchCardTie(this));
            } else if (ownerpoints > visitorpoints) {
                this.broadcast(ShopAndGamePackets.getMatchCardOwnerWin(this));
            } else {
                this.broadcast(ShopAndGamePackets.getMatchCardVisitorWin(this));
            }
            ownerpoints = 0;
            visitorpoints = 0;
        }
    ...
    public void setVisitorPoints() {
        ...
        if (ownerpoints + visitorpoints == matchestowin) {
            if (ownerpoints > visitorpoints) {
                this.broadcast(ShopAndGamePackets.getMiniGameOwnerWin(this));
            } else if (visitorpoints > ownerpoints) {
                this.broadcast(ShopAndGamePackets.getMiniGameVisitorWin(this));
            } else {
                this.broadcast(ShopAndGamePackets.getMiniGameTie(this));
            }
            ownerpoints = 0;
            visitorpoints = 0;
        }
    ...
    public void chat(MapleClient c, String chat) {
        broadcast(ShopAndGamePackets.getPlayerShopChat(c.getPlayer(), chat, isOwner(c.getPlayer())));
    }

    public void sendOmok(MapleClient c, int type) {
        c.getSession().write(ShopAndGamePackets.getMiniGame(c, this, isOwner(c.getPlayer()), type));
    }

    public void sendMatchCard(MapleClient c, int type) {
        c.getSession().write(ShopAndGamePackets.getMatchCard(c, this, isOwner(c.getPlayer()), type));
    }
    ...
    public void setPiece(int move1, int move2, int type, MapleCharacter chr) {
        ...
        if (piece[slot] == 0) {
            piece[slot] = type;
            this.broadcast(ShopAndGamePackets.getMiniGameMoveOmok(this, move1, move2, type));
            for (int y = 0; y < 15; y++) {
                for (int x = 0; x < 11; x++) {
