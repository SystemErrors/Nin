...
public class MapleQuestAction implements Serializable {
	...
	switch (type) {
		...
		for (MapleData iEntry : data.getChildren()) {
			...
			try {
			    MapleInventoryManipulator.removeById(c.getClient(), InventoryConstants.getInventoryType(id), id, (count * -1), true, false);
...
//		break;
	...
	switch (type) {
		...
		for (MapleData iEntry : data.getChildren()) {
			...
			switch (InventoryConstants.getInventoryType(id)) {
			    case EQUIP:
				eq++;
				break;
			    case USE:
