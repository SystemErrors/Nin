...
public class PunishCommands implements InternCommand {
    ...
    public InternCommandDefinition[] getDefinition() {
        return new InternCommandDefinition[]{
                    ...
                    new InternCommandDefinition("tban", "<ign> <timeinhours> <reason>", "Temp ban a person for an Hr.")
        };
    }
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception{
        if (splitted[0].equalsIgnoreCase("unmute")) {
            ...
            if (splitted.length < 3 || originalReason == null) {
                 mc.dropMessage("Syntax helper: !tempban <name> <time in hours> <reason>");
                 return;
            }
            int gReason = 7;
            String reason = c.getPlayer().getName() + " tempbanned " + splitted[1] + ": " + originalReason;
            DateFormat df = DateFormat.getInstance();
            tempB.set(tempB.get(Calendar.YEAR), tempB.get(Calendar.MONTH), tempB.get(Calendar.DATE), tempB.get(Calendar.HOUR_OF_DAY) + duration, tempB.get(Calendar.MINUTE));
