...
public class HelpGMCommand implements GMCommand {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splittedLine) throws Exception {
        ...
        CommandProcessor.getInstance().dropGMHelp(c.getPlayer(), mc, page);
    }

    @Override
    public GMCommandDefinition[] getDefinition() {
        return new GMCommandDefinition[]{
                    new GMCommandDefinition("help", "[page - defaults to 1]", "Shows command list"),
                    new GMCommandDefinition("help", "[page - deafault to 1]", "shows command list")
        };
    }
}
