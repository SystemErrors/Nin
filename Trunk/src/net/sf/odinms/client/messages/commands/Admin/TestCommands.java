...
public class TestCommands implements AdminCommand {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        } else if (splitted[0].equalsIgnoreCase("test1")) {
        } else if (splitted[0].equalsIgnoreCase("test")) {
            MapleCharacter who = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
            MapleRing.createRing(Integer.parseInt(splitted[2]), c.getPlayer(), who, "fff");
            mc.dropMessage("asdf");
        }
    ...
    public AdminCommandDefinition[] getDefinition() {
        return new AdminCommandDefinition[]{
                    new AdminCommandDefinition("tested", "?", "Probably does something"),
                    new AdminCommandDefinition("test", "?", "Probably does something"),
                    new AdminCommandDefinition("clock", "[time]", "Shows a clock to everyone in the map"),};
    }
}
