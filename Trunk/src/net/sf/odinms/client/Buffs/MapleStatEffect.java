...
//	    addBuffStatPairToListIfNotZero(statups, MapleBuffStat.EXPRATE, Integer.valueOf(2)); // EXP
        ...
        if (skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 22181000: // Blessing of the Onyx
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MDEF, (int) ret.mdef));
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.WDEF, (int) ret.wdef));
                    statups.add(new Pair<MapleBuffStat, Integer>(MapleBuffStat.MATK, (int) ret.matk));
                    break;
                case 5121009:
                case 15111005:
    ...
    private final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos) {
        ...
        if (primary) {
            if (itemConNo != 0) {
                MapleInventoryManipulator.removeById(applyto.getClient(), InventoryConstants.getInventoryType(itemCon), itemCon, itemConNo, false, true);
            }
        ...
        } else if (InventoryConstants.isMonsterCard(sourceid)) {
    ...
    public final void applyComboBuff(final MapleCharacter applyto, short combo) {
        final List<Pair<MapleBuffStat, Integer>> stat = Collections.singletonList(new Pair<MapleBuffStat, Integer>(MapleBuffStat.COMBO, (int) combo));
...
//		applyto.getClient().getSession().write(MaplePacketCreator.giveEnergyCharge(stat, (skill ? sourceid : -sourceid), localDuration));
            ...
            case 5001005: // Dash
            case 15001003: {
                applyto.getClient().getSession().write(MaplePacketCreator.giveDash(statups, localDuration));
                applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignDash(statups, localDuration / 1000, applyto.getId()), false);
                normal = false;
                break;
            }
    ...
    private static final int parseMountInfo(final MapleCharacter player, final int skillid) {
        switch (skillid) {
            case 1004: // Monster riding
            case 10001004:
