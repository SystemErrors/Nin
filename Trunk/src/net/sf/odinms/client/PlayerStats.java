...
public class PlayerStats implements Serializable{
    ...
    public final void setAllStats(final int stat) {
        this.str = stat;
        this.dex = stat;
        this.luk = stat;
        this.int_ = stat;
    ...
    public void recalcLocalStats() {
        ...
        buff = chra.getBuffedValue(MapleBuffStat.COMBO); //Aran combo??
        if (buff != null) {
            watk += buff / 10;
        }
        ...
        final ISkill blessoffairy = SkillFactory.getSkill(Skills.getBOF_ForJob(chra.getJob()));
...
//	}
    ...
    public final float calculateMaxBaseDamage(final int watk) {
        ...
        if (watk == 0) {
            ...
            if (weapon_item != null) {
                ...
                final MapleWeaponType weapon = InventoryConstants.getWeaponType(weapon_item.getItemId());
    ...
    public void doAutoAp(byte apla) {
        ...
        byte autoap = c.getAutoAp();
        int remainingAp = c.getRemainingAp();
        switch (autoap) {
            case 0:
                if (remainingAp + apla < Short.MAX_VALUE) {
                    remainingAp += apla;
                    c.updateSingleStat(MapleStat.AVAILABLEAP, remainingAp);
                } else {
                    byte togain = apla;
                    byte gain = (byte) (Short.MAX_VALUE - remainingAp);
                    togain -= gain;
                    c.dropMessage("Your remaining AP has been maxed! Your AP will now be added to Storage AP");
                    c.setAutoAp((byte)5);
                    c.gainAp(gain);
                    c.addStorageAp(togain);
                    c.updateSingleStat(MapleStat.AVAILABLEAP, remainingAp);
                }
                break;
            case 1:
                if (str + apla < Short.MAX_VALUE) {
                    str += apla;
                    c.updateSingleStat(MapleStat.STR, str);
                } else {
                    byte togain = apla;
                    byte gain = (byte) (Short.MAX_VALUE - str);
                    togain -= gain;
                    c.dropMessage("Your STR has been maxed! Your AutoAP has been turned off.");
                    c.setAutoAp((byte)0);
                    str += gain;
                    c.gainAp(togain);
                    c.updateSingleStat(MapleStat.STR, str);
            ...
            case 2:
                if (dex + apla < Short.MAX_VALUE) {
                    dex += apla;
                    c.updateSingleStat(MapleStat.DEX, dex);
                } else {
                    byte togain = apla;
                    byte gain = (byte) (Short.MAX_VALUE - dex);
                    togain -= gain;
                    c.dropMessage("Your DEXTERITY stat has been maxed! Your AutoAP has been turned off.");
                    c.setAutoAp((byte)0);
                    dex += gain;
                    c.gainAp(togain);
                    c.updateSingleStat(MapleStat.DEX, dex);
            ...
            case 3:
                if (int_ + apla < Short.MAX_VALUE) {
                    int_ += apla;
                    c.updateSingleStat(MapleStat.INT, int_);
                } else {
                    byte togain = apla;
                    byte gain = (byte) (Short.MAX_VALUE - int_);
                    togain -= gain;
                    c.dropMessage("Your INT has been maxed! Your AutoAP has been turned off.");
                    c.setAutoAp((byte)0);
                    int_ += gain;
                    c.gainAp(togain);
                    c.updateSingleStat(MapleStat.INT, int_);
            ...
            case 4:
                if (luk + apla < Short.MAX_VALUE) {
                    luk += apla;
                    c.updateSingleStat(MapleStat.LUK, luk);
                } else {
                    int togain = apla;
                    int gain = (byte) (Short.MAX_VALUE - luk);
                    togain -= gain;
                    c.dropMessage("Your LUCK stat has been maxed! Your AutoAP has been turned off.");
                    c.setAutoAp((byte)0);
                    luk += gain;
                    c.gainAp(togain);
                    c.updateSingleStat(MapleStat.LUK, luk);
                ...
                break;
            case 5:
                if (c.getStorageAp() + apla < Integer.MAX_VALUE) {
                    c.addStorageAp(apla);
                } else {
                    byte togain = apla;
                    byte gain = (byte) (Integer.MAX_VALUE - c.getStorageAp());
                    c.showMessage((byte) 5, "Your STORAGE AP has been maxed! You will not gain AP any more.");
                }
                break;
            default:
                if (remainingAp + apla < Short.MAX_VALUE) {
    ...
    public boolean addAllApToStorage(byte type) {
        final MapleCharacter chra = chr.get();
        int ap = getStr() + getDex() + getLuk() + getInt() + chra.getRemainingAp();
        int[] apAllowed = {0, 169, 269, 369, 769, 1019};
        if (ap < apAllowed[type]) {
            chra.dropMessage("You cannot scam me :p. Get " + apAllowed[type] + " before you can try again ");
            return false;
        }
        chra.wipeStats();
        chra.setStorageAp(ap - (apAllowed[type] - 16));
        chra.gainAp(apAllowed[type] - 16);
        chra.dropMessage("Your AP has been successfully added to Storage AP. You now have " + chra.getStorageAp() + " AP in storage");
        return true;
    }
}
