...
public class GiveFameHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (target == c.getPlayer()) { // faming self
            AutobanManager.getInstance().autoban(c, "Trying to Fame himself");
            return;
        } else if (c.getPlayer().getReborns() < 25) {
            c.showMessage(1, "You need to have 25 Rebirths or above to fame some one in NinjaMS");
            return;
        }
