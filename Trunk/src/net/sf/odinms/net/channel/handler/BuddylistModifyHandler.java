...
public class BuddylistModifyHandler extends AbstractMaplePacketHandler {
    ...
    private static void notifyRemoteChannel(MapleClient c, int remoteChannel, int otherCid, BuddyOperation operation)
    ...
    public static void addBuddy(String addName, String group, MapleClient c) {
        WorldChannelInterface worldInterface = c.getChannelServer().getWorldInterface();
        MapleCharacter player = c.getPlayer();
        BuddyList buddylist = player.getBuddylist();
        ...
        if (ble != null && !ble.isVisible()) {
            ...
            try {
                ...
                if (charWithId != null) {
                    ...
                    if (charWithId.gmlvl() > c.getPlayer().getGMLevel()) {
                        c.getSession().write(MaplePacketCreator.getWhisperReply(charWithId.getName(), (byte) 0));
                        return;
                    }
                    if (channel != -1) {
