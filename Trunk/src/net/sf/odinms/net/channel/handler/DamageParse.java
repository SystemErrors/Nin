...
//	}
	if (effect.getAttackCount() != attack.hits || attack.targets > effect.getMobCount()) {
	    player.getCheatTracker().registerOffense(CheatingOffense.MISMATCHING_BULLETCOUNT);
	    return;
	}
	final PlayerStats stats = player.getStat();
...
//	    minDamagePerHit = stats.getTotalMagic() * (stats.getInt() * 0.5 + (v75 * v75) + effect.getMatk() * 3.3) / 100;
	...
	final Integer SharpEye = player.getBuffedSkill_Y(MapleBuffStat.SHARP_EYES);
	final ISkill eaterSkill = SkillFactory.getSkill(Skills.getMPEaterForJob(player.getJob()));
	...
	for (final AttackPair oned : attack.allDamage) {
		...
		if (!Tempest) {
			...
			MaxDamagePerHit = CalculateMaxMagicDamagePerHit(player, theSkill, monster, monsterstats, stats, element, SharpEye, maxDamagePerHit);
		    } else {
			maxDamagePerHit = 1;
		    }
		}
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
	...
	if (MinAccuracy > Accuracy) { // miss :P or HACK :O
	    return 0;
	}
	...
	if (sharpEye != null) {
	    elemMaxDamagePerMob += ((double) elemMaxDamagePerMob / 100) * (sharpEye - 100);
	}
