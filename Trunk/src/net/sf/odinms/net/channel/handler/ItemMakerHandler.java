...
public class ItemMakerHandler {
	...
	switch (makerType) {
		...
		if (InventoryConstants.isGem(toCreate)) {
		    ...
		    if (c.getPlayer().getInventory(InventoryConstants.getInventoryType(randGemGiven)).isFull()) {
		    ...
		    if (c.getPlayer().getInventory(InventoryConstants.getInventoryType(toCreate)).isFull()) {
			return; // We'll do handling for this later
		    }
	...
	for (final Pair p : recipe) {
	    ...
	    MapleInventoryManipulator.removeById(c, InventoryConstants.getInventoryType(itemid), itemid, count, false, false);
	}
	return itemid;
    }    
}