...
//			    if (mobSkill.getCoolTime() == 0) {
	...
	slea.skip(1);
	slea.skip(1); // this byte represent monster type, 1 = walking, 0 = fly OR immo
	slea.skip(3);
	...
	if (res != null) {
	    if (slea.available() != 9) {
		System.out.println("slea.available != 9 (movement parsing error)");
		c.getSession().close();
		return;
	    }
	    /*	    slea.skip(1);
...
//	slea.skip(4); // Some type of pos, damage display, I think
    ...
    public static final void mobDamageMob(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (map.getMonsterByOid(from) != null && map.getMonsterByOid(to) != null) {
            map.getMonsterByOid(to).damage(null, dmg, true);
        }
    }
}
