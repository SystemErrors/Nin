...
public class MapleServerHandler extends IoHandlerAdapter {
    ...
    public void messageReceived(IoSession session, Object message) throws Exception {
     ...
     /*   if (log.isTraceEnabled() || log.isInfoEnabled()) {
        ...
        if (client.getPlayer() != null) {
            if (packetHandler == null && client.getPlayer().isAdmin()) {
                String from = "";
                from = "from " + client.getPlayer().getName() + " ";
                log.info("Got unhandeled Message {} ({}) {}\n{}", new Object[]{from, content.length, HexTool.toString(content), HexTool.toStringFromAscii(content)});
            } else {
        ...
        if (packetHandler != null && packetHandler.validateState(client)) {
            try {
                ...
                // PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO `errorlog` (`error`) VALUES (" + t.getMessage().toString() + ")");
                //ps.execute();
                //ps.close();
            }
        }
